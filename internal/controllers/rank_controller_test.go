...
func TestRankController(t *testing.T) {
	t.Run("Run ranks todo correctly when hideCompleted false", func(t *testing.T) {
		...
		controller := RankController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: false}, Args: []string{"1", "2"}}}
	...
	t.Run("Run ranks todo correctly hideCompleted true", func(t *testing.T) {
		...
		controller := RankController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: true}, Args: []string{"1", "2"}}}
		got, err := controller.Run()
	...
	t.Run("Run returns an error when target index is out of range", func(t *testing.T) {
		...
		controller := RankController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: true}, Args: []string{"1000", "2"}}}
	...
	t.Run("Run returns an error when end index is out of range", func(t *testing.T) {
		...
		controller := RankController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: true}, Args: []string{"1", "2000"}}}
		_, err = controller.Run()
