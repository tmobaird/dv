...
func TestRenameController(t *testing.T) {
	t.Run("Run renames correct todo when hideCompleted false", func(t *testing.T) {
		...
		controller := RenameController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: false}, Args: []string{"1", "Updated Name"}}}
	...
	t.Run("Run renames correct todo when hideCompleted true", func(t *testing.T) {
		...
		controller := RenameController{Base: Controller{Config: internal.Config{Context: "main", HideCompleted: true}, Args: []string{"1", "Updated Name"}}}
		got, err := controller.Run()
	...
	t.Run("Run renames an error when index is out of range", func(t *testing.T) {
		...
		controller := RenameController{Base: Controller{Config: internal.DefaultConfig(), Args: []string{"1000", "Updated Name"}}}
