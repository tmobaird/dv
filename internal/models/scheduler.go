...
func CreateSchedule(date time.Time, calendar Calendar, context string) (string, error) {
	...
	err := os.MkdirAll(internal.ScheduleDirectoryPath(), 0700)
	if err != nil {
		return "", err
	}
	...
	// write to file
	file, err := os.OpenFile(internal.ScheduleFilePath(date), os.O_RDWR|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		return "", err
	}
	defer file.Close()
...
func generateSchedule(start time.Time, events []CalendarEvent, context string) (Schedule, error) {
	...
	todos, err := GetAllTodos(internal.TodoFilePath(context))
	if err != nil {
		return Schedule{}, err
	}
	...
	for (len(availableTasks(todos, todoBank)) > 0 || len(events) > 0) && start.Before(endOfWorkday(time.Now())) {
		...
		todo, index, err := findTask(todos, todoBank, availableTime, prioritizeDeepWork)
...
func availableTasks(todos []Todo, bank map[int]time.Duration) []Todo {
	available := []Todo{}
	for i, todo := range todos {
		if bank[i] > 0 {
...
func findTask(todos []Todo, todoBank map[int]time.Duration, maxDuration time.Duration, prioritizeDeepWork bool) (Todo, int, error) {
